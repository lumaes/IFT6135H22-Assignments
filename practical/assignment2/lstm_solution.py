...
class LSTM(nn.Module):
    ...
    def forward(self, inputs, hidden_states):
        ...
        output, (hn, cn) = self.lstm(embed, (h_prev, c_prev))

        # Classifier
        logits = self.classifier(output)
        logprob_logits = self.logsoftmax(logits)

        return logprob_logits, (hn, cn)
    ...
    def loss(self, log_probas, targets, mask):
        ...
        nll_loss = criterion(log_probas, targets)
        nll_loss = nll_loss * mask
        nll_loss = nll_loss.reshape(default_shape).sum(dim=1)
        nll_loss = (nll_loss / seq_len).mean()
