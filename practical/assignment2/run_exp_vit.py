...
def get_gpu_usage():
  ...
  return round(info.total/1e+9, 3), round(info.free/1e+9,3), round(info.used/1e+9,3)
...
def save_logs(log_dir,
    ...
    log_dir = f'/content/assignment/logs/vit/{exp_id}/'
    os.makedirs(log_dir, exist_ok=True)

    # Log train, val, and test losses and perplexities
    with open(os.path.join(log_dir, "train_loss.json"), "w") as f:
        f.write(str(train_losses))
    with open(os.path.join(log_dir, "train_acc.json"), "w") as f:
        f.write(str(train_accs))
    with open(os.path.join(log_dir, "train_time.json"), "w") as f:
        f.write(str(train_times))
    with open(os.path.join(log_dir, "valid_loss.json"), "w") as f:
        f.write(str(valid_losses))
    with open(os.path.join(log_dir, "valid_acc.json"), "w") as f:
        f.write(str(valid_accs))
    with open(os.path.join(log_dir, "valid_time.json"), "w") as f:
        f.write(str(valid_times))
    with open(os.path.join(log_dir, "test_loss.json"), "w") as f:
        f.write(str(test_loss))
    with open(os.path.join(log_dir, "test_acc.json"), "w") as f:
        f.write(str(test_acc))
    with open(os.path.join(log_dir, "test_time.json"), "w") as f:
