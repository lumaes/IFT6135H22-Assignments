...
class SimSiam(nn.Module):
    ...
    def loss (self, p1,p2,z1,z2, similarity_function='CosineSimilarity'):
        ...
        return - cos(p1, z2).mean()*0.5  - cos(p2, z1).mean()*0.5
...
class CosineSimilarity(Module):
    ...
    def forward(self, x1: Tensor, x2: Tensor) -> Tensor:
        ...
        tmp1 = torch.sum(x1 * x2, self.dim)
        norm1 = torch.norm(x1, 2, self.dim)
        norm2 = torch.norm(x2, 2, self.dim)
        eps = torch.empty(norm1.shape).fill_(self.eps).to(x1.device)
        return (tmp1 / torch.maximum(norm1 * norm2, eps)).squeeze()